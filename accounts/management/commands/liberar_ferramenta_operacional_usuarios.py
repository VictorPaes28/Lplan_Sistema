"""
Concede o grupo «Ferramenta Operacional» (Ferramenta de ambientes) em lote.

Uso:
  python manage.py liberar_ferramenta_operacional_usuarios --dry-run
  python manage.py liberar_ferramenta_operacional_usuarios
  python manage.py liberar_ferramenta_operacional_usuarios --incluir-inativos
"""
...
class Command(BaseCommand):
    help = (
        "Adiciona o grupo «Ferramenta Operacional» a todos os usuários que ainda não têm "
        "acesso à Ferramenta de ambientes."
    )
    ...
    def handle(self, *args, **options):
        ...
        grupo = Group.objects.get(name=GRUPOS.FERRAMENTA_OPERACIONAL)
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                "\n=== Liberar Ferramenta de ambientes (Ferramenta Operacional) ===\n"
            )
        )
