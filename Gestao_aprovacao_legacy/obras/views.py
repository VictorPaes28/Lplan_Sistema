...
from .email_utils import enviar_email_novo_pedido, enviar_email_aprovacao, enviar_email_reprovacao
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        if is_admin(user):
            ...
            # Aprovadores veem pedidos de TODAS as obras das empresas onde têm permissão
            ...
            empresas_ids = Obra.objects.filter(
                id__in=obras_ids
            ...
            # Solicitantes veem pedidos que criaram OU pedidos das obras onde têm permissão
            obras_ids = WorkOrderPermission.objects.filter(
                usuario=user,
                tipo_permissao='solicitante',
                ativo=True
            ...
            # Outros usuários veem apenas pedidos que criaram
...
def list_workorders(request):
    """
    Lista todos os pedidos de obra.
    Solicitantes veem apenas pedidos das obras às quais estão vinculados.
    Gestores veem pedidos das obras sob sua responsabilidade.
    Admins veem todos.
    """
    user = request.user
    user_profile = get_user_profile(user)
    ...
    if is_admin(user):
        ...
        # Aprovadores veem pedidos de TODAS as obras das empresas onde têm permissão
        # Se tem WorkOrderPermission em uma obra, vê TODAS as obras da mesma empresa
        # Buscar todas as obras onde o aprovador tem permissão
        ...
        # Buscar todas as empresas dessas obras
        empresas_ids = obras_com_permissao.values_list('empresa_id', flat=True).distinct()
        
        # Buscar TODAS as obras dessas empresas
        obras_aprovador = Obra.objects.filter(
            empresa_id__in=empresas_ids,
            ativo=True
        ).distinct()
        
        # Mostrar pedidos de TODAS as obras dessas empresas
        ...
        # Solicitantes veem pedidos que criaram
        # Se tem WorkOrderPermission específica, filtra por obras com permissão
        # Se está apenas no grupo "Solicitante", vê todos os pedidos que criou
        ...
        if tem_permissao_solicitante:
            # Se tem permissão específica, filtra por obras com permissão
            ...
            # Ver pedidos criados por este usuário OU por outros solicitantes da mesma obra
            ...
            # Se está apenas no grupo "Solicitante", vê todos os pedidos que criou
            # E também pedidos de outros solicitantes (se estiverem no mesmo grupo)
            outros_solicitantes = User.objects.filter(groups__name='Solicitante').exclude(id=user.id)
            workorders = WorkOrder.objects.filter(
                Q(criado_por=user) | Q(criado_por__in=outros_solicitantes)
            ).select_related('obra', 'obra__empresa', 'criado_por')
            obras_disponiveis = Obra.objects.filter(ativo=True)
...
def create_workorder(request):
    ...
    if not pode_criar:
        ...
        return redirect('list_workorders')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and not anexos_obrigatorios:
            ...
            return redirect('detail_workorder', pk=workorder.pk)
    else:
...
def detail_workorder(request, pk):
    ...
    if is_admin(user):
        ...
        # Aprovador vê se tem permissão em QUALQUER obra da mesma empresa
        # Buscar IDs das empresas onde o aprovador tem permissão
        ...
        # Verificar se a obra do pedido pertence a uma dessas empresas
    ...
    if workorder.pode_aprovar(user):
        if is_admin(user):
            ...
            # Aprovador pode aprovar se tem permissão em QUALQUER obra da mesma empresa
            # Buscar IDs das empresas onde o aprovador tem permissão
            empresas_ids = Empresa.objects.filter(
                obras__permissoes__usuario=user,
                obras__permissoes__tipo_permissao='aprovador',
...
def approve_workorder(request, pk):
    ...
    # Verificar se aprovador tem acesso à empresa (pode aprovar qualquer obra da empresa)
    if not is_admin(request.user):
        ...
        tem_permissao = workorder.obra.empresa_id in empresas_ids
        if not tem_permissao:
            messages.error(request, 'Você não tem permissão para aprovar pedidos desta empresa.')
            return redirect('detail_workorder', pk=workorder.pk)
...
def reject_workorder(request, pk):
    ...
    # Verificar se aprovador tem acesso à empresa (pode reprovar qualquer obra da empresa)
    if not is_admin(request.user):
        # Buscar IDs das empresas onde o aprovador tem permissão
        empresas_ids = Empresa.objects.filter(
            obras__permissoes__usuario=request.user,
        ...
        tem_permissao = workorder.obra.empresa_id in empresas_ids
        if not tem_permissao:
            messages.error(request, 'Você não tem permissão para reprovar pedidos desta empresa.')
            return redirect('detail_workorder', pk=workorder.pk)
    ...
    if request.method == 'POST':
        ...
        # NÃO definir data_aprovacao quando reprovado (apenas quando aprovado)
        # workorder.data_aprovacao = None  # Limpar se existir
        workorder.save()
...
def create_obra(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('detail_obra', pk=obra.pk)
    else:
...
def detail_obra(request, pk):
    ...
    if not (is_admin(request.user) or 
            (is_responsavel_empresa(request.user) and obra.empresa.responsavel == request.user)):
...
def create_user(request):
    ...
    if request.method == 'POST':
        ...
        if not username or not password:
            ...
            # Adicionar vínculos com empresas se fornecidos
            empresas_selecionadas = request.POST.getlist('empresas')
            
            # Se for responsável por empresa (não admin), só pode vincular às suas empresas
            if is_responsavel_empresa(request.user) and not is_admin(request.user):
                empresas_permitidas = Empresa.objects.filter(responsavel=request.user, ativo=True)
                empresas_permitidas_ids = list(empresas_permitidas.values_list('id', flat=True))
                # Filtrar apenas empresas permitidas
                empresas_selecionadas = [eid for eid in empresas_selecionadas if int(eid) in empresas_permitidas_ids]
            
            # Criar vínculos
            empresas_vinculadas_count = 0
            for empresa_id in empresas_selecionadas:
                try:
                    empresa_id_int = int(empresa_id)
                    empresa = Empresa.objects.get(pk=empresa_id_int)
                    # Verificar se é responsável pela empresa (se não for admin)
                    if is_admin(request.user) or (is_responsavel_empresa(request.user) and empresa.responsavel == request.user):
                        UserEmpresa.objects.get_or_create(
                            usuario=user,
                            empresa=empresa,
                            defaults={'ativo': True}
                        )
                        empresas_vinculadas_count += 1
                except (Empresa.DoesNotExist, ValueError, TypeError) as e:
                    # Log do erro para debug (pode remover depois)
                    pass
            ...
            if empresas_vinculadas_count > 0:
                messages.success(request, f'Usuário "{username}" criado com sucesso e vinculado a {empresas_vinculadas_count} empresa(s)!')
            else:
    ...
    # Buscar grupos - criar se não existirem
    grupos = Group.objects.all()
    
    # Se não houver grupos, criar os grupos padrão
    if not grupos.exists():
        grupos_padrao = ['Solicitante', 'Aprovador', 'Responsavel Empresa', 'Administrador']
...
def edit_user(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Remover vínculos antigos que não estão mais selecionados
        if is_admin(request.user):
            # Admin pode gerenciar todos os vínculos
            UserEmpresa.objects.filter(usuario=user).update(ativo=False)
        else:
            # Responsável só pode gerenciar vínculos de suas empresas
            empresas_responsavel = Empresa.objects.filter(responsavel=request.user, ativo=True)
            UserEmpresa.objects.filter(usuario=user, empresa__in=empresas_responsavel).update(ativo=False)
        
        # Adicionar/reativar vínculos selecionados
        for empresa_id in empresas_selecionadas:
            try:
                empresa_id_int = int(empresa_id)
                empresa = Empresa.objects.get(pk=empresa_id_int)
    ...
    if is_responsavel_empresa(request.user) and not is_admin(request.user):
        empresas_disponiveis = Empresa.objects.filter(responsavel=request.user, ativo=True).order_by('codigo')
        # Filtrar apenas empresas onde o usuário editado está vinculado E que são do responsável
    ...
    # Obter perfil do usuário editado
    try:
...
def list_notificacoes(request):
    ...
    if marcar_todas_lidas == 'true':
        ...
        redirect_url = 'list_notificacoes'
        params = []
        if tipo_filter:
...
def marcar_notificacao_lida(request, pk):
    ...
    return redirect('list_notificacoes')
...
def create_empresa(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('detail_empresa', pk=empresa.pk)
    else:
...
def manage_obra_permissions(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('manage_obra_permissions', pk=obra.pk)
    ...
    # Filtrar usuários vinculados à empresa da obra
...
def aprovar_exclusao(request, pk):
    ...
    # Verificar se o aprovador tem permissão na empresa
    if is_aprovador(user) and not is_admin(user):
        ...
        if workorder.obra.empresa_id not in empresas_ids:
            messages.error(request, 'Você não tem permissão para aprovar exclusões desta empresa.')
            return redirect('detail_workorder', pk=workorder.pk)
...
def rejeitar_exclusao(request, pk):
    ...
    if is_aprovador(user) and not is_admin(user):
        ...
        if workorder.obra.empresa_id not in empresas_ids:
            messages.error(request, 'Você não tem permissão para rejeitar exclusões desta empresa.')
            return redirect('detail_workorder', pk=workorder.pk)
    ...
    try:
        ...
        for usuario_id, dados in desempenho_por_usuario.items():
            ...
            if tempo_medio > SLA_HORAS:
                diagnosticos.append(f"Tempo médio acima do SLA ({SLA_HORAS}h)")
            if pct_fora_sla > 30:
                diagnosticos.append(f"Alto percentual fora do SLA ({pct_fora_sla}%)")
            if saldo_capacidade < 0:
                diagnosticos.append("Processo acima da capacidade (backlog crescente)")
            if variacao_tendencia and variacao_tendencia > 20:
                diagnosticos.append("Atraso crescente nos últimos 7 dias")
            if taxa_reprovacao_normalizada > 1.0:
                diagnosticos.append(f"Alta taxa de reprovação ({taxa_reprovacao_normalizada:.1f} reprov./aprov.)")
            if retrabalhos_aprovador > dados['aprovados'] * 0.2:
                diagnosticos.append("Alto índice de retrabalho")
            
            if not diagnosticos:
                diagnostico_texto = "Processo operando dentro dos parâmetros normais"
            else:
                diagnostico_texto = "; ".join(diagnosticos) + f". Risco operacional {classificacao_risco}."
...
def exportar_historico_solicitante(request, solicitante_id):
    ...
    if not (is_admin(request.user) or is_responsavel_empresa(request.user)):
        ...
        return redirect('desempenho_equipe')
...
def reenviar_email(request, log_id):
    ...
    try:
        ...
        return redirect('list_email_logs')
