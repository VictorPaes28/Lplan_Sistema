...
def get_obra_da_sessao(request):
    """Obtém a obra da sessão ou a primeira disponível."""
...
def mapa_engenharia(request):
    ...
    if obra_id:
        # Se mudou via GET, atualizar sessão
        request.session['obra_id'] = int(obra_id)
    else:
        # Usar obra da sessão
...
@login_required
@require_group('ENGENHARIA', 'COMPRAS')
                    ...
                    try:
                        ...
                        try:
                            ...
                            for sheet in xls.sheet_names:
                                ...
                                # Colunas nan no cabeçalho variam entre exports; valores ficam ligados ao rótulo da coluna, não à posição.
...
@login_required
@require_group('ENGENHARIA', 'CHEFIA')
def dashboard_2(request):
    """Dashboard 2 - LPLAN Tactical Command - Pipeline Denso por Local."""
    ...
    if obra_id:
        ...
        if busca_query or filtro_status:
        ...
        if local_selecionado_id:
            ...
            # Padrão: mostrar todos os itens (mesmo layout do Suprimentos)
        ...
        for item in itens_local_selecionado:
            ...
            # Usar quantidade_recebida_obra que já aplica a regra do MAX
            ...
            itens_pipeline.append({
                ...
                'coluna_4_destino_item_qtd': qtd_alocada_item,  # Para tooltip
                # Gaps
        ...
        # Filtro por status/alertas (já aplicado acima, mas reaplicar aqui para itens do local selecionado)
        ...
        if categoria_filtro:
            itens_pipeline = [item for item in itens_pipeline if item.get('categoria') == categoria_filtro]
        ...
        # Busca global (já aplicada acima, mas reaplicar aqui para itens do local selecionado)
        if not busca_query:
            busca_query = request.GET.get('busca', '').strip()
        if busca_query:
            itens_pipeline = [
                item for item in itens_pipeline
                if (busca_query.lower() in item['insumo_descricao'].lower() or
                    busca_query.lower() in (item['insumo_codigo'] or '').lower() or
                    busca_query.lower() in (item['coluna_2_pedido_sc'] or '').lower() or
                    busca_query.lower() in (item['fornecedor'] or '').lower() or
                    busca_query.lower() in (item.get('categoria', '') or '').lower())
            ]
        ...
        if item_selecionado_id:
            try:
                ...
                item_detalhes = {
                    ...
                    'quantidade_planejada': float(item_obj.quantidade_planejada or Decimal('0.00')),
