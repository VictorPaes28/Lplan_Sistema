...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if tem_coluna_obra:
            self.stdout.write(self.style.SUCCESS(f'   ✅ Coluna "Cód. Obra" encontrada - SEGREGAÇÃO AUTOMÁTICA'))
        else:
            ...
            self.stdout.write(self.style.WARNING(
                f'   ⚠️ Usando fallback: {obra_codigo_fallback}'
            ))
        ...
        if not incluir_pequenos:
            self.stdout.write(self.style.SUCCESS(
                f'   ✅ Filtro ativo: Apenas macroelementos serão incluídos no mapa'
            ))
        else:
            self.stdout.write(self.style.WARNING(
                f'   ⚠️ Modo inclusivo: Todos os insumos serão incluídos (incluindo pequenos e cimentos)'
            ))
        
        if insumos_nao_encontrados:
            self.stdout.write(self.style.WARNING(
                f'   ⚠️ Insumos não encontrados (devem ser criados manualmente): {len(insumos_nao_encontrados)} códigos'
            ))
        ...
        with transaction.atomic():
            ...
            for (codigo_obra, numero_sc, codigo_insumo), dados in grupos_sc.items():
                ...
                try:
                    ...
                    # ⚠️ NÃO FAZER ALOCAÇÃO AUTOMÁTICA - apenas armazenar no RecebimentoObra para alocação manual posterior
                    if insumo and numero_sc and dados['quantidade_solicitada'] > Decimal('0.00'):
                        ...
                        if not itens_manuais:
                            ...
                            if not itens_manuais:
                                ...
                                self.stdout.write(
                                    f'      ⚠️ DEBUG: Listando todos os ItemMapa do insumo {insumo.codigo_sienge}:'
                                )
                        ...
                        if itens_manuais:
                            for item_mapa in itens_manuais:
                                ...
                                # ⚠️ IMPORTANTE: NÃO criar AlocacaoRecebimento aqui - apenas atualizar referências
                            ...
                            self.stdout.write(
                                self.style.WARNING(
                                    f'   ⚠️ [{codigo_obra}] SC {numero_sc}, Insumo {insumo.codigo_sienge} ({insumo.descricao[:40]}): '
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f'   📦 Obras: {", ".join(sorted(obras_processadas))}\n'
            f'   🆕 RecebimentoObra criados: {total_recebimentos_criados}\n'
            f'   🔄 RecebimentoObra atualizados: {total_recebimentos_atualizados}\n'
            f'   📋 ItemMapa atualizados: {total_itens_atualizados}\n'
            f'   ⏭️ SC sem ItemMapa encontrado: {total_itens_nao_encontrados}'
        ))
        
        if total_itens_nao_encontrados > 0:
            self.stdout.write(self.style.WARNING(
                f'\n💡 DICA: {total_itens_nao_encontrados} SC(s) não foram encontradas no Mapa (ItemMapa).\n'
