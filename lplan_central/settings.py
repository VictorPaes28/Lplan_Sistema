...
# Em desenvolvimento, garantir que localhost sempre funcione (evita 400)
_local = ['localhost', '127.0.0.1', '[::1]']
for h in _local:
    if h not in ALLOWED_HOSTS:
        ALLOWED_HOSTS.append(h)
if DEBUG:
    ALLOWED_HOSTS = ['*']  # Em dev aceita qualquer host (evita 400 ao acessar por IP/nome da máquina)
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'django.template.context_processors.csrf',  # Necessário para {{ csrf_token }} e forms POST
...
LPLAN_STATIC_VERSION = os.environ.get('LPLAN_STATIC_VERSION', '20260625')
...
# Security Settings
# Em desenvolvimento (DEBUG=True): sem redirect HTTPS, cookies funcionam em HTTP (http://127.0.0.1:8000).
# Em produção (DEBUG=False): defina no .env SECURE_COOKIES_AND_REDIRECT=True para ativar HTTPS.
_use_https = (not DEBUG and
              os.environ.get('SECURE_COOKIES_AND_REDIRECT', 'False').lower() in ('true', '1', 'yes'))
