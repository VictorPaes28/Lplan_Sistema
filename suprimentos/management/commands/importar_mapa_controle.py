...
[!] SC com mesmo insumo em várias linhas:
- Consolidado (``item_sc`` vazio): qt. solicitada e qt. entregue usam a mesma heurística
  (valores iguais em todas as linhas → um valor; diferentes → soma).
- Se o CSV tiver coluna de ITEM da SC: também grava um ``RecebimentoObra`` por item (``item_sc``),
  preservando quantidades e ``status_recebimento`` por entrega; o mapa segue usando o consolidado.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx, row in df.iterrows():
            ...
            if chave not in grupos_sc:
                grupos_sc[chave] = {
                    ...
                    '_linhas_grupo': [],  # snapshots por linha do CSV (item_sc, qtd_sol, qtd_ent)
        ...
        self.stdout.write(
            '      Consolidado (item SC vazio): qt. solicitada e qt. entregue — valores iguais em todas '
            'as linhas → um valor; diferentes (parcelas) → SOMA.'
        )
        self.stdout.write(
                ...
                try:
                    ...
                    if insumo and any((x.get('item_sc') or '').strip() for x in linhas_csv):
                        for L in linhas_csv:
                            ...
                            if importacao:
                                det_defaults['importacao'] = importacao
